...
% [registered, sample, xdata, ydata] = imretrieve(distorted, x(3), x(4), x(1), x(2));
% figure(1),subplot(2,1,1),imshow(original),title('Model');
% subplot(2,1,2),imshow(distorted),title('Scene');
% subplot(2,1,3),imshow(registered),title('Registered Scene');
...
%Run the GA
[x fval reason] = ga(@genetic_function, 1, options);

%Show the registered scene image based on the GA
[registered, sample, xdata, ydata] = imretrieve(distorted, 0, 0, 0, x(1));
